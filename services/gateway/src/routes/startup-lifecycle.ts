...
export function startupLifecycleRoutes(_deps: GatewayDeps) {
  ...
  app.post('/missions/:missionId/checkpoint', async (c) => {
    ...
    const parsed = CreateMissionRuntimeCheckpointInputSchema.safeParse({
      ...(raw as Record<string, unknown>),
      workspaceId,
      missionId: c.req.param('missionId'),
    });
    ...
    const checkpoint = await persistMissionRuntimeCheckpoint(_deps, workspaceId, mission, {
      checkpointKind: parsed.data.checkpointKind,
      reason: parsed.data.reason ?? 'manual mission runtime snapshot',
    });
  ...
  app.post('/missions/:missionId/recover', async (c) => {
    ...
    const checkpoint = await persistMissionRuntimeCheckpoint(_deps, workspaceId, mission, {
      ...
      reason: 'mission recovery requested',
    });
...
async function executeReadyMissionNodes(
  ...
  | { ok: true; response: ExecutedStartupMission }
  | {
      ok: false;
      status: 403 | 404 | 409 | 502;
      body: Record<string, unknown>;
...
async function loadMissionRuntimeSnapshot(
  ...
  const taskIds = taskLinks.map((link) => link.taskId).filter(Boolean);
  const runRows =
    taskIds.length > 0
      ? await deps.db
          .select({
  ...
  const nodeStatusCounts = nodes.reduce<Record<string, number>>((counts, node) => {
