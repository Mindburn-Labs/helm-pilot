...
describe('startupLifecycleRoutes', () => {
  ...
  it('persists mission-level runtime checkpoints with task checkpoint references', async () => {
    const deps = createMockDeps();
    ...
    const readyNodeId = '00000000-0000-4000-8000-000000000151';
    const failedNodeId = '00000000-0000-4000-8000-000000000152';
    const taskId = '00000000-0000-4000-8000-000000000153';
    const taskRunId = '00000000-0000-4000-8000-000000000154';
    const selectResults = [
      [
        {
          id: missionId,
      ...
      [],
      [{ id: '00000000-0000-4000-8000-000000000155', workspaceId, missionId, taskId }],
      [
        {
          id: taskRunId,
          taskId,
          ...
          lastCheckpointAt: new Date('2026-05-06T00:00:00.000Z'),
        },
      ],
    ];
    let selectCall = 0;
    ...
    const body = await expectJson<{
      ...
      taskRunCheckpointRefs: Array<{ taskRunId: string; checkpointId: string }>;
      evidenceItemIds: string[];
      productionReady: boolean;
    }>(res, 201);

    expect(body.checkpointId).toBe('00000000-0000-4000-8000-000000000291');
    expect(body.checkpointKind).toBe('manual_snapshot');
    expect(body.missionStatus).toBe('scheduled_not_executing');
    expect(body.nodeStatusCounts).toEqual({ ready: 1, failed: 1 });
    expect(body.readyNodeIds).toEqual([readyNodeId]);
    expect(body.failedNodeIds).toEqual([failedNodeId]);
    expect(body.taskRunCheckpointRefs).toEqual([
      expect.objectContaining({ taskRunId, checkpointId: 'checkpoint-1' }),
    ]);
    ...
    expect(insertedMissionRuntimeCheckpoints[0]).toMatchObject({
      workspaceId,
      missionId,
      checkpointKind: 'manual_snapshot',
      missionStatus: 'scheduled_not_executing',
      readyNodeIds: [readyNodeId],
    ...
    expect(insertedEvidenceItems[0]).toMatchObject({
      ...
      sourceType: 'gateway_startup_lifecycle',
      replayRef: `mission:${missionId}:checkpoint:manual_snapshot`,
    });
  });
