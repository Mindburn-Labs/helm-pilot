...
- The command-center UI is a `prototype`: it is backed by durable state, including a dedicated browser/computer viewer, permission graph, mission graph with checkpoint/recovery-plan/recovery-apply/rollback evidence, and eval-status inspection routes, but mission runtime is still prototype-only and no Command Center Real-State UX Eval has promoted the surface to `production_ready`.
