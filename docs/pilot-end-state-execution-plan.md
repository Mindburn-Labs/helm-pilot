...
- Subagent spawn rows now carry durable parent/root/spawn anchoring, attached SUBAGENT_SPAWN evidence packs, handoff rows, child action lineage, and `/api/command-center/proof-dag/:taskRunId` inspection. This is `implemented`, not `production_ready`, because founder-facing proof-DAG UI and the Proof DAG Lineage Regression are still missing.
