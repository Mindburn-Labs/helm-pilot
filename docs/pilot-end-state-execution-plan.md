...
- Startup lifecycle templates now compile founder goals into governed lifecycle DAG drafts with required agents, skills, tools, evidence, HELM policy classes, escalation conditions, and acceptance criteria. `/api/startup-lifecycle/persist` stores those DAGs as durable venture, goal, mission, node, edge, and task rows. `/api/startup-lifecycle/missions/:missionId/schedule` identifies dependency-ready nodes and queued task rows. `/api/startup-lifecycle/missions/:missionId/checkpoint` persists a redacted mission/node/edge/task-link snapshot as replayable evidence. `/api/startup-lifecycle/missions/:missionId/recovery-plan` compares the latest checkpoint with current mission state and persists non-executing recovery plan evidence. `/api/startup-lifecycle/missions/:missionId/recover` can reset explicitly failed internal mission nodes to `ready` with recovery-apply evidence without starting execution, rolling back completed work, or touching external systems. `/api/startup-lifecycle/missions/:missionId/nodes/:nodeId/execute` can dispatch one scheduled `ready` node through the governed task runtime with mission context and advance newly unblocked pending nodes to `ready`; `/api/startup-lifecycle/missions/:missionId/execute-ready` can execute a bounded batch of currently ready nodes. This remains `prototype`, not `production_ready`, because the mission DAG is not fully recovered, rolled back, or promoted by Full Startup Launch Eval.
...
- Mission runtime has durable venture, goal, mission, node, edge, task, and mission-task rows plus explicit scheduling, checkpoint snapshots, recovery plan previews, safe failed-node retry reset, and bounded ready-node execution, but still needs full recovery, rollback, replay semantics, and eval promotion.
