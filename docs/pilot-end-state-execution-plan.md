...
- Startup lifecycle templates now compile founder goals into governed lifecycle DAG drafts with required agents, skills, tools, evidence, HELM policy classes, escalation conditions, and acceptance criteria. `/api/startup-lifecycle/persist` stores those DAGs as durable venture, goal, mission, node, edge, and task rows. `/api/startup-lifecycle/missions/:missionId/schedule` identifies dependency-ready nodes and queued task rows. `/api/startup-lifecycle/missions/:missionId/nodes/:nodeId/execute` can dispatch one scheduled `ready` node through the governed task runtime with mission context and advance newly unblocked pending nodes to `ready`; `/api/startup-lifecycle/missions/:missionId/execute-ready` can execute a bounded batch of currently ready nodes; and `/api/startup-lifecycle/missions/:missionId/checkpoint`, `/recover`, and `/rollback` add explicit mission snapshots, recovery planning, and non-destructive constrained rollback. This remains `prototype`, not `production_ready`, because there is still no background founder-off-grid mission loop and the Full Startup Launch Eval has not promoted it.
