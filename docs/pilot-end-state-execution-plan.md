...
- The command-center UI is a `prototype`: it is backed by durable state, including permission graph, mission graph, and eval-status inspection routes, but mission runtime is still blocked and no Command Center Real-State UX Eval has promoted the surface to `production_ready`.
