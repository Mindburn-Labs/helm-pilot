...
- The command-center UI is a `prototype`: it is backed by durable state, including permission graph, mission graph with checkpoint/recovery-plan/recovery-apply evidence, and eval-status inspection routes, but mission runtime is still blocked and no Command Center Real-State UX Eval has promoted the surface to `production_ready`.
...
- Mission runtime has durable venture, goal, mission, node, edge, task, mission-task, and mission checkpoint rows plus explicit scheduling, recovery plan previews, safe failed-node retry reset, and bounded ready-node execution, but still needs full recovery, rollback, replay semantics, and eval promotion.
