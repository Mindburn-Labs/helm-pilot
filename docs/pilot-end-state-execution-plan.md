...
- A canonical `evidence_items` ledger schema now links receipts, tool executions, browser observations, computer actions, artifacts, audit events, and runtime rows. It remains `prototype`, not `production_ready`, because runtime writers do not yet append evidence for every meaningful action and evals have not promoted the capability.
