...
- The Gate 10 eval registry now defines the required production autonomy eval scenarios and `/api/evals/promotion-check` blocks production-ready promotion unless the mapped eval passed with evidence references, audit receipt references, and completion metadata. Eval execution and durable eval-run persistence are still incomplete.
...
- Gate 9, Startup Lifecycle Engine: mission templates/compiler for founder lifecycle workflows with required agents, skills, tools, evidence, policy classes, escalation conditions, and acceptance criteria. Status: current PR, prototype until eval-backed.
- Gate 10, Eval Suite and Production Promotion: production eval scenario registry and promotion guard are present. Durable eval-run persistence, automatic blocker creation, and actual eval execution remain incomplete.
