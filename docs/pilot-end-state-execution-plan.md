...
- A canonical `evidence_items` ledger schema now links receipts, tool executions, browser observations, computer actions, artifacts, audit events, and runtime rows. Core HELM receipt, agent-loop receipt, subagent spawn, browser observation, and computer action writers now append to it, and `/api/command-center` exposes recent evidence items. It remains `prototype`, not `production_ready`, because artifact, eval, integration, lifecycle, and remaining tool writers do not yet append evidence for every meaningful action and evals have not promoted the capability.
