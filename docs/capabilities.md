...
| ---------------------------- | ------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `mission_runtime`            | `blocked`     | Full Startup Launch Eval and Multi-Agent Parallel Build Eval   | Mission checkpoint snapshots and recovery plans are evidence-only; recovery, rollback, and production-ready founder-off-grid DAG automation remain blocked |
...
| `evidence_ledger`            | `prototype`   | HELM Governance Eval and Recovery Eval                         | Core receipt/Tool Broker/browser/computer/connector/pipeline/ingestion/artifact/lifecycle checkpoint/recovery-plan/eval writers append evidence_items, replay endpoints exist, and Tool Broker blocks elevated tools without HELM metadata or evidence persistence, but non-workspace scheduled ingestion and legacy writer coverage is incomplete |
...
| `startup_lifecycle`          | `prototype`   | Full Startup Launch Eval                                       | Lifecycle execution is explicit bounded dispatch with checkpoint snapshots and recovery plan evidence, not a production-ready autonomous startup launch workflow |
