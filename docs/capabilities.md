...
| ---------------------------- | ------------- | -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `mission_runtime`            | `prototype`   | Full Startup Launch Eval and Multi-Agent Parallel Build Eval   | Mission checkpoint, recovery, and rollback controls have not passed production autonomy evals                                  |
...
| `evidence_ledger`            | `prototype`   | HELM Governance Eval and Recovery Eval                         | Core receipt/Tool Broker/browser/computer/connector/pipeline/ingestion/artifact/lifecycle checkpoint/recovery/rollback/eval writers append evidence_items, replay endpoints exist, and Tool Broker blocks elevated tools without HELM metadata or evidence persistence, but non-workspace scheduled ingestion and legacy writer coverage is incomplete |
| `command_center`             | `prototype`   | Command Center Real-State UX Eval                              | Real task/action/evidence/receipt/browser/computer, permission graph, mission graph with checkpoint/recovery evidence, and eval-status rows are visible, but mission DAG autonomy is still prototype-only |
| `startup_lifecycle`          | `prototype`   | Full Startup Launch Eval                                       | Lifecycle execution is explicit bounded dispatch with checkpoint, recovery, and rollback controls, not a production-ready autonomous startup launch workflow |
