...
if [ ! -f .env ] || [ "${overwrite:-}" = "y" ] || [ "${overwrite:-}" = "Y" ]; then
  DB_URL="postgresql://helm:helm@localhost:5432/pilot"
  ...
  cat > .env << EOF
# ─── Pilot Environment (generated by setup.sh) ───
...
if [ "$SKIP_DOCKER" = false ]; then
  ...
  if docker compose -f infra/docker/docker-compose.yml up -d postgres 2>/dev/null; then
    ...
    for i in $(seq 1 30); do
      if docker compose -f infra/docker/docker-compose.yml exec -T postgres pg_isready -U helm -d pilot &>/dev/null; then
        echo ""
...
# Type check
if npx turbo typecheck --filter=@pilot/gateway 2>/dev/null; then
