...
import { users, workspaces, workspaceMembers, operatorRoles, opportunities } from '@pilot/db/schema';
...
async function seed() {
  console.log('Seeding Pilot database...');

  // 1. Create test user
  const [user] = await db
    .insert(users)
    .values({ email: 'dev@pilot.local', name: 'Dev User' })
    .onConflictDoNothing()
    .returning();
