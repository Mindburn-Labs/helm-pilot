...
const capabilityRecords = validateCapabilityRecords([
  ...
  },
  {
    ...
    state: 'prototype',
    summary:
      'A canonical evidence_items schema exists and core HELM receipt, agent-loop receipt, subagent spawn, Tool Broker, browser session control, browser observation, computer action, connector lifecycle, workspace secret mutation, workspace-scoped pipeline/ingestion jobs, artifact creation, startup lifecycle, and eval writers append to it. Tool Broker now fails closed before elevated tool execution without HELM policy metadata and fails elevated completions closed if evidence persistence fails, but evidence coverage is still not complete for every meaningful action.',
    owner: 'Foundation Agent',
    blockers: [
