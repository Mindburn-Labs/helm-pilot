...
const capabilityRecords = validateCapabilityRecords([
  ...
  },
  {
    ...
    state: 'blocked',
    summary:
      'Workspace role enforcement is not strong enough for broad delegated access across governance, secrets, connectors, approvals, operators, and sessions.',
    owner: 'Governance Agent',
    blockers: [
      'No shared requireWorkspaceRole enforcement on all sensitive mutations',
      'Owner/partner/member capabilities are not consistently enforced at route and runtime boundaries',
      'Cross-tenant privilege escalation tests are not yet complete',
    ],
    evidence: ['Gate 2 must enforce route-level and runtime-level RBAC before broad access'],
  ...
  },
  {
    ...
    state: 'blocked',
    summary:
      'operatorId ingress and runtime resolution are not yet proven to reject foreign workspace operators in every autonomous path.',
    owner: 'Governance Agent',
    blockers: [
      'operatorId ownership validation is not centralized',
      'Runtime tool execution can still rely on caller-supplied operator IDs',
      'Foreign workspace operator rejection tests are missing from the baseline gate',
    ],
    evidence: ['Gate 2 must validate operator ownership at ingress and before runtime execution'],
